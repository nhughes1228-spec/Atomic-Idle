...
const upgrades = [
  ...
  { id: "pipette_array", name: "Pipette Array", description: "Active element clicks are 35% stronger.", cost: 520, requires: state => getElementState(state, "H").level >= 12, effect: state => { state.multipliers.click *= 1.35; } },
  { id: "hydrogen_manifold", name: "Hydrogen Manifold", description: "Hydrogen production +50% and element level costs -3%.", cost: 780, requires: state => getElementState(state, "H").level >= 16, effect: state => { state.elementMultipliers.H *= 1.5; state.bonuses.costReduction += 0.03; } },
  ...
  { id: "beryllium_lattice", name: "Beryllium Lattice", description: "Beryllium production x2 and all production +12%.", cost: 92000, requires: state => getElementState(state, "Be").level >= 3, effect: state => { state.elementMultipliers.Be *= 2; state.multipliers.global *= 1.12; } },
  { id: "precision_balancer", name: "Precision Balancer", description: "Active element clicks +40%.", cost: 128000, requires: state => isUnlocked(state, "Be"), effect: state => { state.multipliers.click *= 1.4; } },
  ...
  { id: "publication_pipeline", name: "Publication Pipeline", description: "Future Research gains +35% before publishing.", cost: 5200000000, requires: state => isUnlocked(state, "Ne"), effect: state => { state.bonuses.researchGain *= 1.35; } }
];
